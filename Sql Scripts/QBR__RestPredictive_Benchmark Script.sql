...
UPDATE TRANS_BENCHMARK_RID A
SET A.BENCHMARK_R_ID = B.R_ID
FROM DM_RESTAURANT B
WHERE A.BENCHMARK_RID = B.RID;
...
select
	...
	a.FISHBOWL_STATUS
from STG_ANALYTICS..STG_SF_RESTAURANT_DETAILS a -- main restaurants detail table for fishbowl status and AE info
...
left join 
	(select 
	distinct s.RID,
	sum(case when s.CREATED_DT>date(current_date -  cast('6 month' as interval)) then 1 else 0 end) as Cases6Months,--6 month from current date
	sum(case when s.CREATED_DT<current_date-31 and s.CLOSED_DATE Is NULL then 1 else 0 end) as OpenedCasesOver30days --30 days from current date
	from STG_ANALYTICS..STG_SF_CASES s -- cases data from sales force - case counts
...
left join nscanc_index j on a.RID=j.RID --no show and calcel rates
left join 
	( select a.rid,
...
select 0.01*NEW_RESTO+0.12*H_CHURN_MO+0.23*NO_COVERS+0.15*LOW_OBS
	+0.07*OBS_CHANGE+0.1*MAX_PS_RESTRICTION+0.23*SEARCHES_NA+0.02*SD_CUT_OFF
	+0.34*NO_RR+0.06*RR_SHARE_DECLINE--+0.37*NO_YELP
	+0.01*H_POP+0.02*H_SUB_FEE
	+0.72*NOCONT_6MO+0.17*NO_CASES_6MO+0.25*OPENCASES_OVER30D
	+0.11*H_NO_SHOW+0.25*H_CANCEL+0.38*L_REVIEW_SCORE+0.38*WAS_LOCKED_OUT
	+0.09*NO_PROFILE_IMG+0.03*NO_MENU+0.14*DONT_USE_BOOK as weighted_risk
	, NEW_RESTO, 
	H_CHURN_MO, 
	NO_COVERS, 
	LOW_OBS, 
...
TRUNCATE TABLE ANALYTICS_DW.ADMIN.FCT_QBR_ACTIVITIES;
INSERT INTO ANALYTICS_DW.ADMIN.FCT_QBR_ACTIVITIES
SELECT * FROM MISC_DB..FCT_QBR_ACTIVITIES;

TRUNCATE TABLE ANALYTICS_DW.ADMIN.FCT_QBR_ASSETS;
INSERT INTO ANALYTICS_DW.ADMIN.FCT_QBR_ASSETS
SELECT * FROM MISC_DB..FCT_QBR_ASSETS;

TRUNCATE TABLE ANALYTICS_DW.ADMIN.FCT_QBR_CASES;
INSERT INTO ANALYTICS_DW.ADMIN.FCT_QBR_CASES
SELECT * FROM MISC_DB..FCT_QBR_CASES;

TRUNCATE TABLE ANALYTICS_DW.ADMIN.FCT_QBR_RESTAURANTCONTACTS;
INSERT INTO ANALYTICS_DW.ADMIN.FCT_QBR_RESTAURANTCONTACTS
SELECT * FROM MISC_DB..FCT_QBR_RESTAURANTCONTACTS;

TRUNCATE TABLE ANALYTICS_DW.ADMIN.FCT_REST_PREDICTIVE_ANALYSIS;
INSERT INTO ANALYTICS_DW.ADMIN.FCT_REST_PREDICTIVE_ANALYSIS
SELECT * FROM MISC_DB..FCT_REST_PREDICTIVE_ANALYSIS;

SELECT COUNT(*) FROM FCT_QBR_ACTIVITIES; -- 94051
SELECT COUNT(*) FROM FCT_QBR_ASSETS; -- 103518
SELECT COUNT(*) FROM FCT_QBR_CASES; -- 50633
SELECT COUNT(*) FROM FCT_QBR_RESTAURANTCONTACTS; -- 384748
SELECT COUNT(*) FROM FCT_REST_PREDICTIVE_ANALYSIS; -- 31651