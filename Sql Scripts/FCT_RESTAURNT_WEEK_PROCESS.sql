...
select distinct
	...
	,res.db_name
from analytics_dw..fct_reservation res
join stg_webdb_eu..reservationrestaurantweek resrw on resrw.resid = res.resid 
join stg_webdb_eu..attribution_firstinlastin atr on atr.resid = res.resid
join analytics_dw..DM_RESTAURANT dmres on res.R_ID = dmres.R_ID
join analytics_dw..DM_USERWEB dmuser on res.USERWEB_ID = dmuser.USERWEB_ID
where res.db_name = 'WEBDB_EU';
...
-- left join ANALYTICS_DW..DM_REFERRER Fst on Fst.REFERRERID = fct.ReferrerIDFirstIn and fst.db_name IN ('WEBDB', 'WEBDB_EU') -- INCLUDING WEBDB_EU
-- left join ANALYTICS_DW..DM_REFERRER Lst on Lst.REFERRERID = fct.ReferrerIDLastIn and Lst.db_name IN ('WEBDB', 'WEBDB_EU'); -- INCLUDING WEBDB_EU
left join ANALYTICS_DW..DM_REFERRER Fst on Fst.REFERRERID = fct.ReferrerIDFirstIn and fst.db_name = fct.db_name
left join ANALYTICS_DW..DM_REFERRER Lst on Lst.REFERRERID = fct.ReferrerIDLastIn and Lst.db_name = fct.db_name;
